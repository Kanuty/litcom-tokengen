...
export function TokenPrinter({
  ...
  // Add token(s) to sheet
  const handleAddTokenToSheet = (tokenObj, presetId = null) => {
    ...
    if (existingCount > 0) {
      showNotification({
        ...
        message: `Token "${name}" was added again. Total count on sheet: ${existingCount + (defaultFace === 'both' ? 2 : 1)}.`,
        type: 'info'
      });
    } else {
      showNotification({
        title: 'TOKEN ADDED TO SHEET',
        message: `Added "${name}" to printable page.`,
        type: 'info'
      });
    }

    const newItems = [];
    const baseId = 'st_' + Date.now().toString(36) + '_' + Math.random().toString(36).substring(2, 5);
  ...
  const handleDuplicateTokenOnSheet = (tokenId) => {
    ...
    const newItem = {
      ...target,
      id: newId,
      x: Math.min(paperWidthMM - tokenSizeMM, target.x + 5),
      y: Math.min(paperHeightMM - tokenSizeMM, target.y + 5)
    };
  ...
  // Pointer/Mouse events for Drag and Drop on paper
  ...
  const handleMouseMoveCanvas = (e) => {
    ...
    setSheetTokens((prev) =>
      prev.map((t) => (t.id === draggingId ? { ...t, x: Math.round(newX * 10) / 10, y: Math.round(newY * 10) / 10 } : t))
    );
  };
  ...
  return (
    <div className="token-printer-container" style={{ width: '100%', marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ margin: '0.2rem 0 0 0', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              Arrange tokens onto printable pages with precise gap spacing, margins, and drag & drop positioning.
            </p>
          </div>
      ...
      {/* TWO-COLUMN LAYOUT: CONTROLS & PALETTE | PAPER CANVAS */}
      <div
        style={{
          display: 'grid',
      ...
      >
        {/* LEFT COLUMN: CONTROLS & TOKEN PALETTE */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>

          {/* 1. PAPER & TOKEN SIZE CONFIGURATION */}
          <div className="tint-card tint-card-attributes">
            <h3 className="subsection-header">📄 Paper & Token Dimensions</h3>
          ...
          {/* 2. SPACING & MARGINS (ALL SIDES GAPS) */}
          <div className="tint-card tint-card-grid">
            <h3 className="subsection-header">📏 Gaps & Margin Controls</h3>
            <p className="field-help-text">Set exact spacing between tokens and paper borders from all sides.</p>
            ...
            >
              📐 AUTO-ARRANGE GRID (RE-ALIGN SPACING)
            </button>
          </div>

          {/* 3. TOKEN SELECTION PALETTE */}
          <div className="tint-card tint-card-dice">
            <h3 className="subsection-header">🎯 Token Selection Palette</h3>
            <p className="field-help-text">Click any token to place it on the printable page sheet. You can select one token multiple times!</p>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                <button
                  type="button"
                  onClick={() => handleAddTokenToSheet(activeTokenData, 'active_token')}
                  style={{
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: '0.3rem' }}>
                          <button
                            type="button"
                            onClick={() => handleAddTokenToSheet(item.data, item.id)}
                            style={{
        ...
        {/* RIGHT COLUMN: INTERACTIVE PRINTABLE PAPER SHEET CANVAS */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
              <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>🖐️ Drag tokens on paper to move</span>
            </div>
          </div>
          ...
          >
            ...
            {/* Background Grid Pattern for printable paper visual (Class print-ui-overlay excluded during export) */}
            <svg
              className="print-ui-overlay"
              width="100%"
              height="100%"
              style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none', opacity: 0.15, zIndex: 0 }}
            >
              <defs>
                <pattern id="printer-grid-pattern" width={10 * mmToPx} height={10 * mmToPx} patternUnits="userSpaceOnUse">
                  <path d={`M ${10 * mmToPx} 0 L 0 0 0 ${10 * mmToPx}`} fill="none" stroke="#94a3b8" strokeWidth="0.5" />
                </pattern>
              </defs>
              <rect width="100%" height="100%" fill="url(#printer-grid-pattern)" />
            </svg>
            ...
            {sheetTokens.length === 0 ? (
              ...
              >
                ...
                <div style={{ fontSize: '0.85rem', marginTop: '0.3rem' }}>Select tokens from the left panel to place them on this page!</div>
              </div>
            ) : (
